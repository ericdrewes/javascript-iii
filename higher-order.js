...
let evenNumbers = mixedNumbers.filter((i) => {
  return i % 2 === 0;
})
...
let myStrongest = monstersInYourPocket.filter((i) => {
      return i["CP"] > 200
});
...
let ordersTotal = orders.map((i) => {
      return i["price"] += i["price"] * i["tax"] 
});
...
let bobsTotal = purchases.reduce((acc, cur) => {
      if(cur["owner"] === "Bob")
      {
